...
namespace TagTool.Scripting.Compiler
{
	public class ScriptCompiler
	{
		...
		public void AppendCompileFile(FileInfo file) {
			...
			MemoryStream memoryStream = new MemoryStream(Definition.ScriptStrings);
		...
		public void CompileFile(FileInfo file) {
			...
			foreach (IScriptSyntax node in nodes) {
		...
		private HsScriptType ParseScriptType(IScriptSyntax node) {
			if (!( node is ScriptSymbol symbol ) ||
				!Enum.TryParse<HsScriptType>(symbol.Value, true, out HsScriptType result)) {
				throw new FormatException(node.ToString());
			}
		...
		private uint CompileStringAddress(string input) {
			if (!StringOffsets.ContainsKey(input)) {
				uint offset = (uint)StringWriter.BaseStream.Position;
		...
		private void CompileGlobal(IScriptSyntax node) {
			...
			HsType globalType = ParseHsType(declGroup.Head);
			...
			string globalName = declName.Value;
			...
			DatumHandle globalInit = CompileExpression(globalType.HaloOnline, declTailTailGroup.Head);
		...
		private void PrecompileScript(IScriptSyntax node) {
			...
			HsScriptType scriptType = ParseScriptType(declGroup.Head);
			...
			HsType scriptReturnType = ParseHsType(declTailGroup.Head);
			...
			if (!skipReturnType && !( declTailGroup.Tail is ScriptGroup )) {
				...
				declTailTailGroup = skipReturnType ? declTailGroup : (ScriptGroup)declTailGroup.Tail;
			}

			string scriptName;
			List<HsScriptParameter> scriptParams = new List<HsScriptParameter>();
			...
			switch (declTailTailGroup.Head) {
				...
				case ScriptGroup declNameGroup: {
					...
					for (IScriptSyntax param = declParamGroup;
						...
						HsType paramType = ParseHsType(paramDeclType);
						...
						string paramName = paramDeclName.Value;
		...
		private DatumHandle CompileExpression(HsType.HaloOnlineValue type, IScriptSyntax node) {
			...
			if (node is ScriptSymbol symbol) {
				...
				if (CurrentScript != null) {
					foreach (HsScriptParameter parameter in CurrentScript?.Parameters) {
						
						if (parameter.Name != symbol.Value) { continue; }
						...
						DatumHandle retValue = CompileParameterReference(symbol, parameter);

						// Ensure the parameter is reset to its original state if it was modified
						if (modified) { parameter.Type.HaloOnline = originalParamType; }
				...
				foreach (HsGlobal global in Globals) {

					if (global.Name != symbol.Value) { continue; }
					...
					DatumHandle retValue = CompileGlobalReference(symbol, global);

					// Ensure the parameter is reset to its original state if it was modified
					if (modified) { global.Type.HaloOnline = originalParamType; }

					return retValue;

				}

				foreach (KeyValuePair<int, string> global in ScriptInfo.Globals[(Cache.Version, Cache.Platform)]) {
		...
		private DatumHandle AllocateExpression(HsType.HaloOnlineValue valueType, HsSyntaxNodeFlags expressionType, ushort? opcode = null, short? line = null) {
			...
			HsSyntaxNode expression = new HsSyntaxNode
			{
		...
		private DatumHandle CompileGroupExpression(HsType.HaloOnlineValue type, ScriptGroup group) {
			...
			switch (functionNameSymbol.Value) {
				case "begin":
				case "begin_random": {
					KeyValuePair<int, ScriptInfo> builtin = ScriptInfo.Scripts[(Cache.Version, Cache.Platform)].First(x => x.Value.Name == functionNameSymbol.Value);

					DatumHandle firstHandle = DatumHandle.None;
					...
					for (IScriptSyntax current = group.Tail;
						...
						DatumHandle currentHandle = CompileExpression(HsType.HaloOnlineValue.Unparsed, currentGroup.Head);
					...
					DatumHandle beginHandle = AllocateExpression(type, HsSyntaxNodeFlags.Group, (ushort)builtin.Key, (short)group.Line);
					HsSyntaxNode beginExpr = ScriptExpressions[beginHandle.Index];

					DatumHandle functionNameHandle = AllocateExpression(HsType.HaloOnlineValue.FunctionName, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, (ushort)builtin.Key, (short)functionNameSymbol.Line);
					HsSyntaxNode functionNameExpr = ScriptExpressions[functionNameHandle.Index];
				...
				case "if": {
					...
					DatumHandle ifHandle = AllocateExpression(type, HsSyntaxNodeFlags.Group, (ushort)builtin.Key, (short)group.Line);
					HsSyntaxNode ifExpr = ScriptExpressions[ifHandle.Index];
					// if opcode
					DatumHandle functionNameHandle = AllocateExpression(HsType.HaloOnlineValue.FunctionName, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, (ushort)builtin.Key, (short)functionNameSymbol.Line);
					HsSyntaxNode functionNameExpr = ScriptExpressions[functionNameHandle.Index];
					functionNameExpr.StringAddress = CompileStringAddress(functionNameSymbol.Value);
					...
					DatumHandle booleanHandle = CompileExpression(HsType.HaloOnlineValue.Boolean, booleanGroup.Head);
					HsSyntaxNode booleanExpr = ScriptExpressions[booleanHandle.Index];
					...
					DatumHandle thenHandle = CompileExpression(type, thenGroup.Head);
					HsSyntaxNode thenExpr = ScriptExpressions[thenHandle.Index];
					...
					ScriptGroup nextGroup = thenGroup;
					HsSyntaxNode nextExpr = thenExpr;
				...
				case "cond": {
					...
					DatumHandle condHandle = AllocateExpression(type, HsSyntaxNodeFlags.Group, (ushort)builtin.Key, (short)group.Line);
					HsSyntaxNode condExpr = ScriptExpressions[condHandle.Index];
					...
					IScriptSyntax current = group.Tail;

					if (!( current is ScriptGroup ) && !( current is ScriptInvalid )) {
						throw new FormatException(group.ToString());
					}

					HsSyntaxNode prevExpr = functionNameExpr;
					...
					while (current is ScriptGroup currentGroup) {
						...
						DatumHandle booleanGroupHandle = AllocateExpression(type, HsSyntaxNodeFlags.Group, line: (short)condGroup.Line);
						HsSyntaxNode booleanGroupExpr = ScriptExpressions[booleanGroupHandle.Index];

						DatumHandle booleanHandle = CompileExpression(HsType.HaloOnlineValue.Boolean, condGroup.Head);
						HsSyntaxNode booleanExpr = ScriptExpressions[booleanHandle.Index];
				...
				case "set": {
					...
					foreach (HsGlobal global in Globals) {
						...
						DatumHandle setHandle = AllocateExpression(global.Type.HaloOnline, HsSyntaxNodeFlags.Group, (ushort)builtin.Key, (short)group.Line);
						HsSyntaxNode setExpr = ScriptExpressions[setHandle.Index];
						...
						DatumHandle globalHandle = CompileGlobalReference(globalName, global);
						functionNameExpr.NextExpressionHandle = globalHandle;

						HsSyntaxNode globalExpr = ScriptExpressions[globalHandle.Index];
				...
				case "or": {
					...
					DatumHandle handle = AllocateExpression(builtin.Value.Type, HsSyntaxNodeFlags.Group, (ushort)builtin.Key, (short)group.Line);
					HsSyntaxNode expr = ScriptExpressions[handle.Index];
					...
					for (IScriptSyntax current = group.Tail;
						...
						DatumHandle currentHandle = CompileExpression(HsType.HaloOnlineValue.Boolean, currentGroup.Head);
				...
				case "max": {
					...
					for (IScriptSyntax current = group.Tail;
						...
						DatumHandle currentHandle = DatumHandle.None;
				...
				case ">=": {
					...
					HsSyntaxNode firstExpr = ScriptExpressions[functionNameExpr.NextExpressionHandle.Index];
				...
				case "sleep": {
					...
					HsSyntaxNode lengthExpr = ScriptExpressions[functionNameExpr.NextExpressionHandle.Index];
				...
				case "sleep_until": {
					...
					if (tailGroup.Tail is ScriptGroup tailTailGroup) {
						HsSyntaxNode booleanExpr = ScriptExpressions[functionNameExpr.NextExpressionHandle.Index];
						...
						if (tailTailGroup.Tail is ScriptGroup tailTailTailGroup) {
							...
							HsSyntaxNode tickExpr = ScriptExpressions[booleanExpr.NextExpressionHandle.Index];
			...
			foreach (KeyValuePair<int, ScriptInfo> entry in ScriptInfo.Scripts[(Cache.Version, Cache.Platform)]) {
				if (functionNameSymbol.Value != entry.Value.Name) {
					continue;
				}

				// "objects_distance_to_flag" is handled here; Allocate Expression
				// At this point it has the correct data types associated with the input parameters
				// The DoNotGarbageCollect flag is applied to the resulting HsSyntaxNode regardless of the op type

				DatumHandle handle = AllocateExpression(entry.Value.Type, HsSyntaxNodeFlags.Group, (ushort)entry.Key, (short)functionNameSymbol.Line);
				HsSyntaxNode expr = ScriptExpressions[handle.Index];

				DatumHandle functionNameHandle = AllocateExpression(HsType.HaloOnlineValue.FunctionName, HsSyntaxNodeFlags.Expression, (ushort)entry.Key, (short)functionNameSymbol.Line);
				HsSyntaxNode functionNameExpr = ScriptExpressions[functionNameHandle.Index];
				functionNameExpr.StringAddress = CompileStringAddress(functionNameSymbol.Value);
				...
				foreach (ScriptInfo.ParameterInfo parameter in entry.Value.Parameters) {
			...
			foreach (HsScript script in Scripts) {
				...
				DatumHandle handle = AllocateExpression(
				...
				DatumHandle functionNameHandle = AllocateExpression(
				...
				foreach (HsScriptParameter parameter in script.Parameters) {
					if (!( parameters is ScriptGroup parametersGroup )) {
						throw new FormatException(group.ToString());
					}
		...
		private DatumHandle CompileExternMethodReference(ScriptGroup group, ScriptSymbol functionNameSymbol, HsScript script) {
			KeyValuePair<int, ScriptInfo> builtin = ScriptInfo.Scripts[(Cache.Version, Cache.Platform)].First(x => x.Value.Name == "dew_method_stub");
			short scriptIndex = (short)Scripts.IndexOf(script);

			DatumHandle handle = AllocateExpression(script.ReturnType.HaloOnline, HsSyntaxNodeFlags.Group |  HsSyntaxNodeFlags.Extern, (ushort)builtin.Key, scriptIndex);
			HsSyntaxNode expr = ScriptExpressions[handle.Index];

			DatumHandle functionNameHandle = AllocateExpression(HsType.HaloOnlineValue.FunctionName, HsSyntaxNodeFlags.Expression | HsSyntaxNodeFlags.Extern, (ushort)builtin.Key, scriptIndex);
			HsSyntaxNode functionNameExpr = ScriptExpressions[functionNameHandle.Index];
			functionNameExpr.StringAddress = CompileStringAddress(functionNameSymbol.Value);
		...
		private DatumHandle CompileGlobalReference(ScriptSymbol symbol, HsGlobal global) {
			DatumHandle handle = AllocateExpression(global.Type.HaloOnline, HsSyntaxNodeFlags.GlobalsReference, line: (short)symbol.Line);
		...
		private DatumHandle CompileGlobalReference(ScriptSymbol symbol, HsType.HaloOnlineValue type, string name, ushort opcode) {
			DatumHandle handle = AllocateExpression(type, HsSyntaxNodeFlags.GlobalsReference, line: (short)symbol.Line);
		...
		private DatumHandle CompileParameterReference(ScriptSymbol symbol, HsScriptParameter parameter) {
			DatumHandle handle = AllocateExpression(parameter.Type.HaloOnline, HsSyntaxNodeFlags.ParameterReference, line: (short)symbol.Line);
		...
		private DatumHandle CompileBooleanExpression(ScriptBoolean boolValue) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.Boolean, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)boolValue.Line);
		...
		private DatumHandle CompileRealExpression(ScriptReal realValue) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.Real, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)realValue.Line);
		...
		private DatumHandle CompileShortExpression(ScriptInteger shortValue) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.Short, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)shortValue.Line);
		...
		private DatumHandle CompileLongExpression(ScriptInteger longValue) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.Long, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)longValue.Line);
		...
		private DatumHandle CompileStringExpression(ScriptString stringValue) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.String, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)stringValue.Line);
		...
		private DatumHandle CompileScriptExpression(ScriptSymbol scriptSymbol) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.Script, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)scriptSymbol.Line);

			if (handle != DatumHandle.None) {
				int scriptIndex = Scripts.FindIndex(s => s.ScriptName == scriptSymbol.Value);
		...
		private DatumHandle CompileStringIdExpression(ScriptString stringIdString) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.StringId, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)stringIdString.Line);

			if (handle != DatumHandle.None) {
				StringId stringId = Cache.StringTable.GetStringId(stringIdString.Value);
		...
		private DatumHandle CompileUnitSeatMappingExpression(ScriptString unitSeatMappingString) {
			...
			if (handle != DatumHandle.None) {
				// TODO: Get unit_seat_mapping data

				// Figure out how to get these values
				int unitSeatStartIndex = 0;
				int unitSeatMappingCount = 0;
		...
		private DatumHandle CompileTriggerVolumeExpression(ScriptString triggerVolumeString) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.TriggerVolume, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)triggerVolumeString.Line);

			if (handle != DatumHandle.None) {
				int triggerVolumeIndex = Definition.TriggerVolumes.FindIndex(tv => triggerVolumeString.Value == Cache.StringTable.GetString(tv.Name));
		...
		private DatumHandle CompileCutsceneFlagExpression(ScriptString cutsceneFlagString) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.CutsceneFlag, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)cutsceneFlagString.Line);

			if (handle != DatumHandle.None) {
				int cutsceneFlagIndex = Definition.CutsceneFlags.FindIndex(cf => cutsceneFlagString.Value == Cache.StringTable.GetString(cf.Name));
		...
		private DatumHandle CompileCutsceneCameraPointExpression(ScriptString cutsceneCameraPointString) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.CutsceneCameraPoint, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)cutsceneCameraPointString.Line);

			if (handle != DatumHandle.None) {
				int cutsceneCameraPointIndex = Definition.CutsceneCameraPoints.FindIndex(ccp => cutsceneCameraPointString.Value == ccp.Name);
		...
		private DatumHandle CompileCutsceneTitleExpression(ScriptSymbol cutsceneTitleSymbol) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.CutsceneTitle, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)cutsceneTitleSymbol.Line);

			if (handle != DatumHandle.None) {
				int cutsceneTitleIndex = Definition.CutsceneTitles.FindIndex(ct => cutsceneTitleSymbol.Value == Cache.StringTable.GetString(ct.Name));
		...
		private DatumHandle CompileDeviceGroupExpression(ScriptString deviceGroupString) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.DeviceGroup, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)deviceGroupString.Line);

			if (handle != DatumHandle.None) {
				int deviceGroupIndex = Definition.DeviceGroups.FindIndex(dg => dg.Name == deviceGroupString.Value);
		...
		private DatumHandle CompileAiExpression(ScriptString aiString) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.Ai, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)aiString.Line);

			if (handle != DatumHandle.None) {
				string[] tokens = aiString.Value.Split('/');
				int value = 0;
				...
				switch (tokens.Length) {
					case 1: {
						...
						int squadIndex = Definition.Squads.FindIndex(s => s.Name == tokens[0]);
						...
						int squadGroupIndex = Definition.SquadGroups.FindIndex(sg => sg.Name == tokens[0]);
						...
						int objectiveIndex = Definition.AiObjectives.FindIndex(o => tokens[0] == Cache.StringTable.GetString(o.Name));
					...
					case 2: {
						...
						if (squadIndex != -1) {
							Scenario.Squad squad = Definition.Squads[squadIndex];

							//
							// type 4: spawn point
							//

							int spawnPointIndex = squad.SpawnPoints.FindIndex(sp => tokens[1] == Cache.StringTable.GetString(sp.Name));
							...
							int spawnFormationIndex = squad.SpawnFormations.FindIndex(sf => tokens[1] == Cache.StringTable.GetString(sf.Name));
						...
						if (objectiveIndex != -1) {
							int taskIndex = Definition.AiObjectives[objectiveIndex].Tasks.FindIndex(t => tokens[1] == Cache.StringTable.GetString(t.Name));
		...
		private DatumHandle CompileAiLineExpression(ScriptString aiLineString) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.AiLine, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)aiLineString.Line);

			if (handle != DatumHandle.None) {
				StringId lineStringId = Cache.StringTable.GetStringId(aiLineString.Value);
		...
		private DatumHandle CompileStartingProfileExpression(ScriptString startingProfileString) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.StartingProfile, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)startingProfileString.Line);

			if (handle != DatumHandle.None) {
				int startingProfileIndex = Definition.PlayerStartingProfile.FindIndex(sp => sp.Name == startingProfileString.Value);
		...
		private DatumHandle CompileZoneSetExpression(ScriptString zoneSetString) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.ZoneSet, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)zoneSetString.Line);

			if (handle != DatumHandle.None) {
				int zoneSetIndex = Definition.ZoneSets.FindIndex(zs => zoneSetString.Value == Cache.StringTable.GetString(zs.Name));
		...
		private DatumHandle CompileDesignerZoneExpression(ScriptString designerZoneString) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.ZoneSet, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)designerZoneString.Line);

			if (handle != DatumHandle.None) {
				int designerZoneIndex = Definition.DesignerZoneSets.FindIndex(dz => designerZoneString.Value == Cache.StringTable.GetString(dz.Name));
		...
		private DatumHandle CompilePointReferenceExpression(ScriptString pointReferenceString) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.PointReference, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)pointReferenceString.Line);

			if (handle != DatumHandle.None) {
				string[] tokens = pointReferenceString.Value.Split('/');

				if (tokens.Length != 2) {
					throw new FormatException(pointReferenceString.Value);
				}

				int pointSetIndex = Definition.ScriptingData[0].PointSets.FindIndex(ps => ps.Name == tokens[0]);

				if (pointSetIndex == -1) {
					throw new FormatException(pointReferenceString.Value);
				}

				int pointIndex = Definition.ScriptingData[0].PointSets[pointSetIndex].Points.FindIndex(p => p.Name == tokens[1]);
		...
		private DatumHandle CompileObjectListExpression(ScriptString objectListString) {
			...
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.ObjectList, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, objectOpcode, line: (short)objectListString.Line);

			if (handle != DatumHandle.None) {
				int objectIndex = objectListString.Value == "none" ? -1 :
		...
		private DatumHandle CompileFolderExpression(ScriptString folderString) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.Folder, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)folderString.Line);

			if (handle != DatumHandle.None) {
				int folderIndex = folderString.Value == "none" ? -1 : Definition.EditorFolders.FindIndex(ef => ef.Name == folderString.Value);
		...
		private DatumHandle CompileSoundExpression(ScriptString soundString) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.Sound, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)soundString.Line);

			if (handle != DatumHandle.None) {
				if (!Cache.TagCache.TryGetTag<Sound>(soundString.Value, out CachedTag instance)) {
		...
		private DatumHandle CompileEffectExpression(ScriptString effectString) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.Effect, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)effectString.Line);

			if (handle != DatumHandle.None) {
				if (!Cache.TagCache.TryGetTag<Effect>(effectString.Value, out CachedTag instance)) {
		...
		private DatumHandle CompileDamageExpression(ScriptString damageString) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.Damage, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)damageString.Line);

			if (handle != DatumHandle.None) {
				if (!Cache.TagCache.TryGetTag<DamageEffect>(damageString.Value, out CachedTag instance)) {
		...
		private DatumHandle CompileLoopingSoundExpression(ScriptString loopingSoundString) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.LoopingSound, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)loopingSoundString.Line);

			if (handle != DatumHandle.None) {
				if (!Cache.TagCache.TryGetTag<SoundLooping>(loopingSoundString.Value, out CachedTag instance)) {
		...
		private DatumHandle CompileAnimationGraphExpression(ScriptString animationGraphString) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.AnimationGraph, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)animationGraphString.Line);

			if (handle != DatumHandle.None) {
				if (!Cache.TagCache.TryGetTag<ModelAnimationGraph>(animationGraphString.Value, out CachedTag instance)) {
		...
		private DatumHandle CompileDamageEffectExpression(ScriptString damageEffectString) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.DamageEffect, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)damageEffectString.Line);

			if (handle != DatumHandle.None) {
				if (!Cache.TagCache.TryGetTag<DamageEffect>(damageEffectString.Value, out CachedTag instance)) {
		...
		private DatumHandle CompileObjectDefinitionExpression(ScriptString objectDefinitionString) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.ObjectDefinition, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)objectDefinitionString.Line);

			if (handle != DatumHandle.None) {
				if (!Cache.TagCache.TryGetTag(objectDefinitionString.Value, out CachedTag instance) || !instance.IsInGroup("obje")) {
		...
		private DatumHandle CompileBitmapExpression(ScriptString bitmapString) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.Bitmap, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)bitmapString.Line);

			if (handle != DatumHandle.None) {
				if (!Cache.TagCache.TryGetTag<Bitmap>(bitmapString.Value, out CachedTag instance)) {
		...
		private DatumHandle CompileShaderExpression(ScriptString shaderString) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.Shader, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)shaderString.Line);

			if (handle != DatumHandle.None) {
				if (!Cache.TagCache.TryGetTag<RenderMethod>(shaderString.Value, out CachedTag instance)) {
		...
		private DatumHandle CompileRenderModelExpression(ScriptString renderModelString) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.RenderModel, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)renderModelString.Line);

			if (handle != DatumHandle.None) {
				if (!Cache.TagCache.TryGetTag<RenderModel>(renderModelString.Value, out CachedTag instance)) {
		...
		private DatumHandle CompileStructureDefinitionExpression(ScriptString structureDefinitionString) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.StructureDefinition, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)structureDefinitionString.Line);

			if (handle != DatumHandle.None) {
				if (!Cache.TagCache.TryGetTag<ScenarioStructureBsp>(structureDefinitionString.Value, out CachedTag instance)) {
		...
		private DatumHandle CompileCinematicDefinitionExpression(ScriptString cinematicDefinitionString) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.CinematicDefinition, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)cinematicDefinitionString.Line);

			if (handle != DatumHandle.None) {
				if (!Cache.TagCache.TryGetTag<Cinematic>(cinematicDefinitionString.Value, out CachedTag instance)) {
		...
		private DatumHandle CompileCinematicSceneDefinitionExpression(ScriptString cinematicSceneDefinitionString) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.CinematicSceneDefinition, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)cinematicSceneDefinitionString.Line);

			if (handle != DatumHandle.None) {
				if (!Cache.TagCache.TryGetTag<CinematicScene>(cinematicSceneDefinitionString.Value, out CachedTag instance)) {
		...
		private DatumHandle CompileBinkDefinitionExpression(ScriptString binkDefinitionString) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.BinkDefinition, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)binkDefinitionString.Line);

			if (handle != DatumHandle.None) {
				if (!Cache.TagCache.TryGetTag<Bink>(binkDefinitionString.Value, out CachedTag instance)) {
		...
		private DatumHandle CompileAnyTagExpression(ScriptString anyTagString) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.AnyTag, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)anyTagString.Line);

			if (handle != DatumHandle.None) {
				if (!Cache.TagCache.TryGetTag(anyTagString.Value, out CachedTag instance)) {
		...
		private DatumHandle CompileAnyTagNotResolvingExpression(ScriptString anyTagNotResolvingString) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.AnyTagNotResolving, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)anyTagNotResolvingString.Line);

			if (handle != DatumHandle.None) {
				if (!Cache.TagCache.TryGetTag(anyTagNotResolvingString.Value, out CachedTag instance)) {
		...
		private DatumHandle CompileGameDifficultyExpression(ScriptSymbol gameDifficultySymbol) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.GameDifficulty, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)gameDifficultySymbol.Line);

			if (handle != DatumHandle.None) {
				if (!Enum.TryParse<GameDifficulty>(gameDifficultySymbol.Value, true, out GameDifficulty difficulty)) {
		...
		private DatumHandle CompileTeamExpression(ScriptSymbol teamSymbol) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.Team, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)teamSymbol.Line);

			if (handle != DatumHandle.None) {
				if (!Enum.TryParse<GameTeam>(teamSymbol.Value, true, out GameTeam team)) {
		...
		private DatumHandle CompileMpTeamExpression(ScriptSymbol mpTeamSymbol) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.MpTeam, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)mpTeamSymbol.Line);

			if (handle != DatumHandle.None) {
				if (!Enum.TryParse<GameMultiplayerTeam>(mpTeamSymbol.Value, true, out GameMultiplayerTeam mpTeam)) {
		...
		private DatumHandle CompileControllerExpression(ScriptSymbol controllerSymbol) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.Controller, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)controllerSymbol.Line);

			if (handle != DatumHandle.None) {
				if (!Enum.TryParse<GameController>(controllerSymbol.Value, true, out GameController controller)) {
		...
		private DatumHandle CompileButtonPresetExpression(ScriptSymbol buttonPresetSymbol) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.ButtonPreset, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)buttonPresetSymbol.Line);

			if (handle != DatumHandle.None) {
				if (!Enum.TryParse<GameControllerButtonPreset>(buttonPresetSymbol.Value, true, out GameControllerButtonPreset buttonPreset)) {
		...
		private DatumHandle CompileJoystickPresetExpression(ScriptSymbol joystickPresetSymbol) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.JoystickPreset, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)joystickPresetSymbol.Line);

			if (handle != DatumHandle.None) {
				if (!Enum.TryParse<GameControllerJoystickPreset>(joystickPresetSymbol.Value, true, out GameControllerJoystickPreset joystickPreset)) {
		...
		private DatumHandle CompilePlayerCharacterTypeExpression(ScriptSymbol playerCharacterTypeSymbol) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.PlayerCharacterType, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)playerCharacterTypeSymbol.Line);

			if (handle != DatumHandle.None) {
				if (!Enum.TryParse<GamePlayerCharacterType>(playerCharacterTypeSymbol.Value, true, out GamePlayerCharacterType playerCharacterType)) {
		...
		private DatumHandle CompileVoiceOutputSettingExpression(ScriptSymbol voiceOutputSettingSymbol) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.VoiceOutputSetting, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)voiceOutputSettingSymbol.Line);

			if (handle != DatumHandle.None) {
				if (!Enum.TryParse<GameVoiceOutputSetting>(voiceOutputSettingSymbol.Value, true, out GameVoiceOutputSetting voiceOutputSetting)) {
		...
		private DatumHandle CompileVoiceMaskExpression(ScriptSymbol voiceMaskSymbol) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.VoiceMask, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)voiceMaskSymbol.Line);

			if (handle != DatumHandle.None) {
				if (!Enum.TryParse<GameVoiceMask>(voiceMaskSymbol.Value, true, out GameVoiceMask voiceMask)) {
		...
		private DatumHandle CompileSubtitleSettingExpression(ScriptSymbol subtitleSettingSymbol) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.SubtitleSetting, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)subtitleSettingSymbol.Line);

			if (handle != DatumHandle.None) {
				if (!Enum.TryParse<GameSubtitleSetting>(subtitleSettingSymbol.Value, true, out GameSubtitleSetting subtitleSetting)) {
		...
		private DatumHandle CompileActorTypeExpression(ScriptSymbol actorTypeSymbol) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.ActorType, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)actorTypeSymbol.Line);

			if (handle != DatumHandle.None) {
				if (!Enum.TryParse<ActorTypeEnum>(actorTypeSymbol.Value, true, out ActorTypeEnum actorType)) {
		...
		private DatumHandle CompileModelStateExpression(ScriptSymbol modelStateSymbol) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.ModelState, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)modelStateSymbol.Line);

			if (handle != DatumHandle.None) {
				if (!Enum.TryParse<GameModelState>(modelStateSymbol.Value, true, out GameModelState modelState)) {
		...
		private DatumHandle CompileEventExpression(ScriptSymbol eventSymbol) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.Event, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)eventSymbol.Line);

			if (handle != DatumHandle.None) {
				if (!Enum.TryParse<GameEventType>(eventSymbol.Value, true, out GameEventType eventType)) {
		...
		private DatumHandle CompileCharacterPhysicsExpression(ScriptSymbol characterPhysicsSymbol) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.CharacterPhysics, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)characterPhysicsSymbol.Line);

			if (handle != DatumHandle.None) {
				if (!Enum.TryParse<GameCharacterPhysics>(characterPhysicsSymbol.Value, true, out GameCharacterPhysics characterPhysics)) {
		...
		private DatumHandle CompilePrimarySkullExpression(ScriptSymbol primarySkullSymbol) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.PrimarySkull, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)primarySkullSymbol.Line);

			if (handle != DatumHandle.None) {
				if (!Enum.TryParse<GamePrimarySkull>(primarySkullSymbol.Value, true, out GamePrimarySkull primarySkull)) {
		...
		private DatumHandle CompileSecondarySkullExpression(ScriptSymbol secondarySkullSymbol) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.SecondarySkull, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)secondarySkullSymbol.Line);

			if (handle != DatumHandle.None) {
				if (!Enum.TryParse<GameSecondarySkull>(secondarySkullSymbol.Value, true, out GameSecondarySkull secondarySkull)) {
		...
		private DatumHandle CompileObjectExpression(ScriptString objectString) {
			...
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.Object, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, objectOpcode, line: (short)objectString.Line);

			if (handle != DatumHandle.None) {
				int objectIndex = objectString.Value == "none" ? -1 :
		...
		private DatumHandle CompileUnitExpression(ScriptString unitString) {
			...
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.Unit, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, unitOpcode, line: (short)unitString.Line);

			if (handle != DatumHandle.None) {
				int unitIndex = unitString.Value == "none" ? -1 :
		...
		private DatumHandle CompileVehicleExpression(ScriptString vehicleString) {
			...
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.Vehicle, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, vehicleOpcode, line: (short)vehicleString.Line);

			if (handle != DatumHandle.None) {

				int vehicleIndex = vehicleString.Value == "none" ? -1 :
		...
		private DatumHandle CompileWeaponExpression(ScriptString weaponString) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.Weapon, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)weaponString.Line);

			if (handle != DatumHandle.None) {
				int weaponIndex = weaponString.Value == "none" ? -1 :
		...
		private DatumHandle CompileDeviceExpression(ScriptString deviceString) {
			...
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.Device, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, deviceOpcode, line: (short)deviceString.Line);

			if (handle != DatumHandle.None) {
				int deviceIndex = deviceString.Value == "none" ? -1 :
		...
		private DatumHandle CompileSceneryExpression(ScriptString sceneryString) {
			...
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.Scenery, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, sceneryOpcode, line: (short)sceneryString.Line);

			if (handle != DatumHandle.None) {
				int sceneryIndex = sceneryString.Value == "none" ? -1 :
		...
		private DatumHandle CompileEffectSceneryExpression(ScriptString effectSceneryString) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.EffectScenery, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)effectSceneryString.Line);

			if (handle != DatumHandle.None) {
				int effectSceneryIndex = effectSceneryString.Value == "none" ? -1 :
		...
		private DatumHandle CompileObjectNameExpression(ScriptString objectNameString) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.ObjectName, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)objectNameString.Line);

			if (handle != DatumHandle.None) {
				int objectNameIndex = Definition.ObjectNames.FindIndex(on => on.Name == objectNameString.Value);
		...
		private DatumHandle CompileUnitNameExpression(ScriptString objectNameString) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.UnitName, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)objectNameString.Line);
		...
		private DatumHandle CompileVehicleNameExpression(ScriptString objectNameString) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.VehicleName, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)objectNameString.Line);
		...
		private DatumHandle CompileWeaponNameExpression(ScriptString objectNameString) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.WeaponName, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)objectNameString.Line);
		...
		private DatumHandle CompileDeviceNameExpression(ScriptString objectNameString) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.DeviceName, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)objectNameString.Line);
		...
		private DatumHandle CompileSceneryNameExpression(ScriptString objectNameString) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.SceneryName, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)objectNameString.Line);
		...
		private DatumHandle CompileEffectSceneryNameExpression(ScriptString objectNameString) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.EffectSceneryName, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)objectNameString.Line);
		...
		private DatumHandle CompileCinematicLightprobeExpression(ScriptSymbol cinematicLightprobeSymbol) {
			DatumHandle handle = AllocateExpression(HsType.HaloOnlineValue.CinematicLightprobe, HsSyntaxNodeFlags.Primitive | HsSyntaxNodeFlags.DoNotGC, line: (short)cinematicLightprobeSymbol.Line);

			if (handle != DatumHandle.None) {
				int cinematicLightprobeIndex = Definition.CinematicLighting.FindIndex(cl => cinematicLightprobeSymbol.Value == Cache.StringTable.GetString(cl.Name));
		...
		private void WriteTagToSourceFileReferences(ScriptString tagString) {
			if (Cache.TagCache.TryGetTag(tagString.Value, out CachedTag instance)) {
				...
				foreach (TagReferenceBlock tagEntry in ScriptSourceFileReferences) {
